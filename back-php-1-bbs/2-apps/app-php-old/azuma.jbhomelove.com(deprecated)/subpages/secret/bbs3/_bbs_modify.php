<?php
  ...
  if(isset($img1)) {
      ...
      if(move_uploaded_file($tmp_name1, $dir.$file_name1)) {
          // 파일 입력 성공시
          require('_conn.php');
          $sql = "UPDATE `sub_secret_bbs3` SET `title` = '$title', `content` = '$content', `nickname` = '$nickname', `update_date` = CURRENT_TIMESTAMP, `img_file1` = '$file_name1', `img_size1` = '$img_size1' WHERE `sub_secret_bbs3`.`no`=$no";
      ...
      // 파일 입력 실패시
          ...
          require('_conn.php');
          $sql = "UPDATE `sub_secret_bbs3` SET `title` = '$title', `content` = '$content', `nickname` = '$nickname', `update_date` = CURRENT_TIMESTAMP WHERE `sub_secret_bbs3`.`no`=$no";
          mysqli_query($conn, $sql);
          header("refresh:0.1; url=bbs.php");
          }
